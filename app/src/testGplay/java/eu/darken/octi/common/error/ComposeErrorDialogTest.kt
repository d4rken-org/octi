...
class ComposeErrorDialogTest : BaseTest() {
    ...
    fun `a fixable error offers its action next to a dismiss`() {
        ...
        composeRule.onNodeWithText(context.getString(CommonR.string.general_dismiss_action)).assertExists()
        // Nothing is being cancelled or merely acknowledged here.
        composeRule.onAllNodesWithText(context.getString(CommonR.string.general_cancel_action)).assertCountEquals(0)
    ...
    fun `dismissing closes the dialog without launching anything`() {
        ...
        composeRule.onAllNodesWithText(context.getString(CommonR.string.general_dismiss_action)).assertCountEquals(0)
    ...
    @Test
    fun `an error with an infoAction offers show details`() {
        // The other negative-button branch: no fix to offer, but details to reveal.
        var infoClicks = 0
        showError(InfoActionError(onInfo = { infoClicks++ }))
