...
class GplayUpgradeScreenTest : BaseComposeRobolectricTest() {
    ...
    fun `loaded state keeps unavailable actions visible but disabled`() {
        ...
        // Tag, not text: octi's iap offer title and iap action button share the copy "One-time
        // purchase", so a text lookup for the button is ambiguous when the price row falls back to
        // its bare title.
        ...
        // Two nodes in octi: the bare iap offer title AND the iap button label are both "One-time
        // purchase" (shared copy). The exact match still proves the title has no dangling "·".
        composeRule.onAllNodesWithText(context.getString(R.string.upgrade_screen_iap_offer_title)).assertCountEquals(2)
    ...
    fun `non-renewing subscription owner can buy the one-time upgrade`() {
        ...
        composeRule.onAllNodesWithText(context.getString(R.string.upgrade_screen_switch_body)).assertCountEquals(1)
    ...
    fun `aged grace stage shows diagnostics with an inline restore action`() {
        ...
        composeRule.onAllNodesWithText(context.getString(R.string.upgrade_screen_grace_diagnostics_body)).assertCountEquals(1)
    ...
    fun `the inconclusive dialog offers a retry that fires the callback`() {
        ...
        composeRule.onNodeWithText(context.getString(CommonR.string.general_refresh_action)).performClick()
