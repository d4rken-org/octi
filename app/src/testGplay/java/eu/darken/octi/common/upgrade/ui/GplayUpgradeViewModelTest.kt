...
class GplayUpgradeViewModelTest : BaseTest() {
    ...
    fun `restore that finds a purchase emits RestoreSucceeded`() = runTest2(context = testDispatcher) {
        val repo = mockRepo()
        coEvery { repo.restorePurchaseNow() } returns checked(proInfo(mockPurchase(OurSku.Iap.PRO_UPGRADE.id)))
        val vm = buildVm(repo)
    ...
    fun `default route bounces a pro user out of the screen`() = runTest2(context = testDispatcher) {
        val repo = mockRepo()
        every { repo.upgradeInfo } returns MutableStateFlow(proInfo(mockPurchase(OurSku.Sub.PRO_UPGRADE.id, autoRenewing = true)))
        val vm = buildVm(repo)
    ...
    fun `successful queries never render from an unsettled Info`() = runTest2(
        ...
        infos.value = proInfo(mockPurchase(OurSku.Sub.PRO_UPGRADE.id))
        advanceUntilIdle()
    ...
    fun `settled owner renders ownership while queries are still pending`() = runTest2(
        ...
        val repo = mockRepo()
        every { repo.upgradeInfo } returns MutableStateFlow(proInfo(mockPurchase(OurSku.Sub.PRO_UPGRADE.id)))
        coEvery { repo.querySkus(any()) } coAnswers {
