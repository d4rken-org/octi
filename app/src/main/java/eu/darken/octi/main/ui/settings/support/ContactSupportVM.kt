...
class ContactSupportVM @Inject constructor(
        ...
        sessionManager.state
            ...
            .onEach { managerState ->
                stater.updateBlocking {
                    val justStopped = isRecording && !managerState.isRecording
                    val autoSelected = if (justStopped) {
                        managerState.sessions.firstOrNull()?.session
                    } else {
                        null
                    }
                    val selected = autoSelected ?: selectedSession
                    val cappedSessions = managerState.sessions.take(MAX_PICKER_SESSIONS)
                    val selectedStillExists = selected == null ||
                        managerState.isRecording ||
                        managerState.sessions.any { it.session.sessionDir == selected.sessionDir }
                    copy(
                        isRecording = managerState.isRecording,
                        sessions = cappedSessions,
                        selectedSession = if (selectedStillExists) selected else null,
                    )
                }
            }
            .launchInViewModel()
    ...
    fun sendEmail() = launch {
        val currentState = stater.updateBlocking { copy(isSending = true) }
        try {
            ...
            val zipUri = currentState.selectedSession?.let { session ->
                try {
                    ...
                    // A cancelled send must never continue into the chooser as a "successful" email.
                    throw e
                } catch (e: Exception) {
