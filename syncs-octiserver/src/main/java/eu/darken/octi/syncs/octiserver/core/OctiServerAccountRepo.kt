...
@Singleton
class OctiServerAccountRepo @Inject constructor(
    ...
    private val _accounts = DynamicStateFlow(parentScope = scope + dispatcherProvider.Default) {
        dataStore.data.first()
            ...
            .map { json.decodeFromString<OctiServer.Credentials>(it) }
            .toSet()
    }

    val accounts: Flow<Collection<OctiServer.Credentials>> = _accounts.flow

    suspend fun add(acc: OctiServer.Credentials): Boolean {
        log(TAG) { "add(acc=$acc)" }
        var added = false
    ...
    suspend fun remove(id: OctiServer.Credentials.AccountId) {
    ...
    companion object {
        ...
        private val TAG = logTag("Sync", "OctiServer", "AccountRepo")
    }
}
