...
class ViewModel4StateFlowTest : BaseTest() {
    ...
    fun `safeStateIn forwards failure and keeps fallback state collectable`() = runTest2(
        ...
        forwardedError.await().shouldBeInstanceOf<StateCollectionException>()
    ...
    private class StateCollectionException : IllegalStateException("No matching mode available.")
    ...
    private class FailingStateViewModel(
        ...
        val state = flow {
            emit(1)
            throw StateCollectionException()
        }.safeStateIn(
            initialValue = 0,
            onError = { -1 },
        )
