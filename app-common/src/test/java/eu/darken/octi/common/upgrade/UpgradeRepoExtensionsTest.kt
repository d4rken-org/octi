...
class UpgradeRepoExtensionsTest : BaseTest() {
    ...
    // region isHardLocked (widget hard-lock boundary)

    @Test
    fun `isHardLocked is true only for a settled error-free non-pro state`() {
        // Pro is never a lock.
        FakeInfo(isPro = true, isSettled = true).isHardLocked() shouldBe false
        // A billing error must not present as a hard lock (retry, not a false lock).
        FakeInfo(isPro = false, isSettled = true, error = IllegalStateException("boom")).isHardLocked() shouldBe false
        // Cold-start seed (not yet settled) is "checking", not locked.
        FakeInfo(isPro = false, isSettled = false).isHardLocked() shouldBe false
        // The only true hard lock: settled, error-free, no purchase.
        FakeInfo(isPro = false, isSettled = true).isHardLocked() shouldBe true
    }
