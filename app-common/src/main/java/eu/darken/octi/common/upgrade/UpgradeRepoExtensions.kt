...
/** Definitively, error-free-settled non-Pro — the widget hard-lock boundary. */
fun UpgradeRepo.Info.isHardLocked(): Boolean = error == null && isSettled && !isPro

/**
 * Pro check for backend safety-net gates (e.g. an upload-execution boundary).
 *
 ...
 *   purchase (the realistic "free user reached a Pro-only path" case, where billing is already
 *   connected).
...
/**
 * Maps [UpgradeRepo.upgradeInfo] to a [ProState] stream suitable for UI gating (notably the widget
 * config screens).
 *
 * The four Info shapes map distinctly so a paying user is never hard-locked by a transient state:
 * - Pro (active purchase or grace) → [ProState.Unlocked].
 * - A settled error → [ProState.Error]; widgets can offer a retry instead of a false lock.
 * - Not yet settled (GPlay cold-start seed) → [ProState.Checking]; render a loading state rather
 *   than momentarily showing Locked to a paying user during cold-start billing queries.
 * - Settled, error-free, no purchase → [ProState.Locked], the only true hard lock.
 *
 * [ProState.Checking] is also seeded before the first emission lands. Collection errors translate
 * into [ProState.Error]; the upstream is left intact so any downstream `.catch { … }` still
 * observes the throwable if needed.
 */
fun UpgradeRepo.proState(): Flow<ProState> = upgradeInfo
    .map { info ->
        val error = info.error
        when {
            info.isPro -> ProState.Unlocked
            error != null -> ProState.Error(error)
            !info.isSettled -> ProState.Checking
            else -> ProState.Locked
        }
    }
