...
Octi Server is an end to end encrypted open-source sync server hosted by me.
...
The connectivity module determines your device's public IP address by sending a request to an Octi Server. If you have configured a custom Octi Server, that server is used first. Otherwise, the app falls back to the default Octi Server provided by me. The server inherently sees your device's IP address as part of processing the request. No additional data is collected or stored through this process.
